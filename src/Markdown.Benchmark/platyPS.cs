...
namespace Markdown.Benchmark
{
    ...
    public class PlatyPS
    {
        ...
        public void LoadMarkdown()
        {
            ...
            _Command = PipelineBuilder.ToMamlCommand().Process(_CommandMarkdown, path: null);
            _Topic = PipelineBuilder.ToAboutTopic().Process(_CommandMarkdown, path: null);
        }

        [Benchmark]
        public MamlCommand ToMamlCommand() => PipelineBuilder.ToMamlCommand().Process(_CommandMarkdown);

        [Benchmark]
        public string ToMamlXml() => PipelineBuilder.ToMamlXml().Process(new[] { _Command });

        [Benchmark]
        public string ToMarkdown() => PipelineBuilder.ToMarkdown().Process(_Command);
